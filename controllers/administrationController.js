...
controller.addUser = async (req, res) => {
  try {
    const {
      'first-name': firstName, 'last-name': lastName, email, language, status, 'user-designation': userDesignation,
      isAdmin, projectId, 'access-type': accessType, locale, timezone, 
    } = req.body;

  
    const image = req.file ? req.file.filename : 'DefaultPhoto.png';
...
controller.editUser = async (req, res) => {
  try {
    ...
    const {
      'first-name': firstName, 'last-name': lastName, email, language, status, 'user-designation': userDesignation,
      isAdmin, locale, timezone, 'user-img': oldImage
    } = req.body;

    const image = req.file ? req.file.filename : 'DefaultPhoto.png';

    if (req.file && oldImage && oldImage !== 'DefaultPhoto.png') {
...
controller.deleteUser = async (req, res) => {
  try {
    ...
    if (userImg && userImg !== 'DefaultPhoto.png') {
